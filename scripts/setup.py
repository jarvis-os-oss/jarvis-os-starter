#!/usr/bin/env python3
"""AI-OS Starter Kit: first-run onboarding.

Generates a local ``.env`` from ``.env.example`` by asking for the values a new
instance needs (cockpit token, provider key, domain), runs the agent naming
step, and can wire the ``upstream`` git remote for one-way updates from this
starter repo.
...
NAMES_EXAMPLE = os.path.join(ROOT, "agents", "names.example")
NAMES_LOCAL = os.path.join(ROOT, "agents", "names.local")
NAME_SCRIPT = os.path.join(ROOT, "scripts", "name_agents.py")
...
def name_agents(interactive=True):
    """Naming step: let the user give each agent its own name, then apply it.

    Seeds ``agents/names.local`` from ``names.example`` on first run, asks for a
    name per agent (Enter keeps the generic role default), and applies the
    result to both the persona files and the roster via
    ``scripts/name_agents.py``. The applier is idempotent, so this is safe to
    re-run. Non-interactive mode simply applies whatever names.local (or the
    role defaults) already contain, so a fresh CI install still resolves every
    ``{{AGENT_NAME}}`` placeholder to the role name.
    """
    try:
    ...
    if interactive:
        print("[setup] Name your agents. Press Enter to keep the role default.")
        chosen = {}
        for a in agents:
            key = a.get("key")
            cur = a.get("name", key)
            try:
                got = input(f"  {key} [{cur}]: ").strip()
            except EOFError:
                got = ""
            chosen[key] = got or cur
        # Persist the choices to the git-ignored names.local so a later re-run
        # (or `name_agents.py`) reproduces them.
        with open(NAMES_LOCAL, "w") as fh:
            fh.write("# Your chosen agent names (generated by scripts/setup.py).\n")
            fh.write("# Edit and re-run: python3 scripts/name_agents.py\n")
            for a in agents:
                key = a.get("key")
                fh.write(f"{key} = {chosen[key]}\n")
        print(f"[setup] Wrote {os.path.relpath(NAMES_LOCAL, ROOT)} (git-ignored).")

    # Apply names (from names.local if present, else the role defaults) to the
    # persona files and the roster. Delegates to the dedicated, tested applier.
    try:
        subprocess.run([sys.executable, NAME_SCRIPT], check=True)
    except Exception as e:
        print(f"[setup] naming step failed: {e}")
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="AI-OS Starter Kit setup")
    ...
    ap.add_argument("--skip-naming", "--skip-rename", dest="skip_naming",
                    action="store_true",
                    help="skip the agent naming step (personas keep the role "
                         "default names)")
    ...
    if not args.skip_naming:
        name_agents(interactive=interactive)
